...
pub async fn qtpsend(
    ...
    let (conn, _incoming_messages) = incoming_conns.next().await.expect("cannot get connection");
    //let (mut sendstream, _recvstream) = conn.open_bi().await.expect("cannot open stream");
    //audio connection stream
    let (conn2, _incoming_messages2) = incoming_conns2.next().await.expect("cannot get connection");
    ...
    tokio::spawn(async move {
        loop {
            let bytes = incoming_messages3.next().await.expect("cannot get data");
            if bytes != None {
