...
pub async fn qtpreceive(
    ...
    let peer2 = peer.ip();
    let portaudio = peer.port() + 1 as u16;
    let portinput = peer.port() + 2 as u16;
    ...
    let (_connection2, mut incoming_messages2) = node2
        .connect_to(&SocketAddr::new(peer2, portaudio))
        .await
        .expect("cannot create connection");
    let (sent_stream_input, _incoming_messages3) = node3
        .connect_to(&SocketAddr::new(peer2, portinput))
        .await
        .expect("cannot create connection");
        ...
        gstreamer_app::AppSrcCallbacks::builder()
            .need_data(move |appsrc3, _| {
                pollster::block_on(async {
                    ...
                    if bytes.clone() != None {
                        match appsrc3.push_buffer(gst::Buffer::from_slice(bytes.expect("msg"))) {
                            Ok(_data) => {
                                //   println!("Flow buffer receive")
                            }
                            Err(_err) => {
                                // println!("Flow buffer does not receive")
                            }
                        }
                    }
                });
        ...
        gstreamer_app::AppSrcCallbacks::builder()
            .need_data(move |appsrcaudio2, _| {
                pollster::block_on(async {
                    ...
                    if bytes.clone() != None {
                        match appsrcaudio2.push_buffer(gst::Buffer::from_slice(bytes.expect("msg")))
                        {
                            Ok(_data) => {
                                // println!("Flow buffer receive")
                            }
    ...
    let cursor_position = Arc::new(AtomicPtr::from(Box::new(PhysicalPosition::new(0.0, 0.0))));
