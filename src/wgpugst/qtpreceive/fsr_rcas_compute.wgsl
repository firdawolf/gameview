...
fn main(@builtin(workgroup_id) WorkGroupID : vec3<u32>,
  ...
  @builtin(local_invocation_id) LocalInvocationID : vec3<u32>) {
    ...
    let inputdimension : vec2<i32> = textureDimensions(input, 0);
	...
	let b : vec3<f32> = textureLoad(input,baseIndex + vec2<i32>(0, -inputdimension.y),0).rgb;
	let d : vec3<f32> = textureLoad(input,baseIndex + vec2<i32>(-inputdimension.x, 0),0).rgb;
	var e : vec3<f32> = textureLoad(input,baseIndex,0).rgb;
	let f : vec3<f32> = textureLoad(input,baseIndex + vec2<i32>(inputdimension.x, 0),0).rgb;
	let h : vec3<f32> = textureLoad(input,baseIndex + vec2<i32>(0, inputdimension.y),0).rgb;
